...
translationOf: /zh/docs/guide/content/
type: doc
group: Guide
order: 20
...
Wiki links, embeds, callouts, and `%%` comments are enabled by default and can be toggled separately through `markdown.obsidian` in `site.config.mjs`. [[docs/guide/configuration|View site configuration]].
...
![[docs/guide/configuration#Page transitions]]
